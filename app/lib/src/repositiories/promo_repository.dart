...
class PromoRepository {
  /// Урамшууллын мэдээллүүдийг авах
  Future<List> fetchNewPromorion() async {
    ...
    try {
      ...
      if (decoded['isSuccess']) {
        promos = List<NewPromoMdl>.from(decoded['promotions']
            .map((promoMdl) => NewPromoMdl.fromJson(promoMdl)));
      } else {
        promos = null;
      }
      return promos;
...
class AntennRepository {
  ...
  Future<List> fetchAntenna() async {
    ...
    try {
      ...
      var decoded = json.decode(response);
      if (decoded['isSuccess']) {
        manuals = List<AntennMdl>.from(decoded['manuals']
            .map((antennMdl) => AntennMdl.fromJson(antennMdl)));
      } else {
        manuals = null;
      }
      return manuals;
...
class AntennVideoRepository {
  ///Антенн тохируулах заавар видеонууд авах
  Future<List> fetchAntennaVideo() async {
    ...
    try {
      ...
      var decoded = json.decode(response);
      if (decoded['isSuccess']) {
        vimanuals = List<AntennVideoMdl>.from(decoded['videoManuals']
            .map((antennVideoMdl) => AntennVideoMdl.fromJson(antennVideoMdl)));
      } else {
        vimanuals = null;
      }
      return vimanuals;
